...
# Version: 9.3 (Decoupled Offloads for Flawless bsdconfig Integration)
...
SYS_VERSION=$(uname -r)
BASE_RELEASE=$(uname -r | cut -d'-' -f1-2) # Strip patch level
...
# ------------------------------------------------------------------------------
# 4. PERSISTENCE AND BUG WORKAROUNDS (DECOUPLED FOR BSDCONFIG)
# ------------------------------------------------------------------------------
...
chmod 555 "$RC_SCRIPT"
add_line_if_missing 'aq_offloads_enable="YES"' /etc/conf
